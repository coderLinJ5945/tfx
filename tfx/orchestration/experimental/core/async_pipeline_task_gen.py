...
class _Generator:
  ...
  def _generate_tasks_for_node(self, metadata_handler: metadata.Metadata,
    ...
    if oldest_active_execution:
      if backfill_token:
        if oldest_active_execution.custom_properties[
          ...
          logging.exception(
              ...
              'was: %s. Aborting backfill and setting node to STOPPED state',
              node.node_info.id, backfill_token, oldest_active_execution)
          result.append(
              task_lib.UpdateNodeStateTask(
                  node_uid=node_uid,
    ...
    if backfill_token:
      successful_executions = [
          ...
          if e.custom_properties['backfill'].string_value == backfill_token
      ]
    ...
    if not unprocessed_inputs:
      if backfill_token:
        logging.info(
            ...
            'process, so backfill is complete. Setting node to STOPPED state.',
            node.node_info.id)
        result.append(
            task_lib.UpdateNodeStateTask(
                node_uid=node_uid,
    ...
    if backfill_token:
      if len(unprocessed_inputs) != 1:
        # We can support backfills where the node resolves multiple inputs,
        # (we just need to set the backfill token on all executions), but for
        # now we will reject such cases until we see a need for them.
        logging.exception(
            'Node %s run in backfill mode, but it resolved %d inputs, instead '
            'of exactly one input. For now, nodes run in backfill mode must '
            'resolve exactly one input. Aborting backfill and setting node to '
            'STOPPED state.', node.node_info.id, len(unprocessed_inputs))
        result.append(
            task_lib.UpdateNodeStateTask(
                node_uid=node_uid,
                state=pstate.NodeState.STOPPED,
