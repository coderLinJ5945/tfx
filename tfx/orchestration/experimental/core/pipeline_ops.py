  ...
  with pstate.PipelineState.load(mlmd_handle,
    ...
    with pipeline_state.node_state_update_context(node_uid) as node_state:
      if node_state.backfill_token:
        raise status_lib.StatusNotOkError(
            code=status_lib.Code.INVALID_ARGUMENT,
            message=(f'Node {node_uid} is already in backfill mode with token '
                     f'{node_state.backfill_token}. If you want to abort the '
                     f'backfill and start a new one, stop the node first.'))

      if node_state.state == pstate.NodeState.STOPPED:
        ...
        raise status_lib.StatusNotOkError(
            code=status_lib.Code.INVALID_ARGUMENT,
            message=(f'Can only backfill nodes in a stopped state, '
